...
@section('content')
    <div class="container-fluid">
        <div class="d-sm-flex align-items-center justify-content-between mb-0">
            <h1 class="h3 mb-0 text-gray-800 menu-title">Detail Hutang - {{ $accountPayable->number }}</h1>
        </div>
        @if ($errors->any())
                                    ...
                                    <tbody class="table-ar" id="itemTable" >
                                        @foreach($accountPayablePayments as $index => $accountPayablePayment)
                                            <tr class="table-modal-first-row text-dark" id="{{ $index }}" style="font-size: 16px !important">
                                                ...
                                                <td class="text-right">
                                                    <input type="text" class="form-control form-control-sm text-bold text-dark text-right" name="payment_amount[]" id="paymentAmount-{{ $index }}" value="{{ formatPrice($accountPayablePayment->amount) }}" data-toogle="tooltip" data-placement="bottom" title="Hanya masukkan angka saja" style="font-size: 16px">
