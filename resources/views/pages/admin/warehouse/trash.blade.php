...
@section('content')
    <div class="container-fluid">
        <div class="d-sm-flex align-items-center justify-content-between mb-2">
            <div>
                <h1 class="h3 mb-0 text-gray-800 menu-title">List of Deleted Warehouses</h1>
            </div>
            <div class="row">
                <form action="{{ route('warehouses.restore', 0) }}" method="POST">
                    @csrf
                    @method('PUT')
                    <button type="submit" class="btn btn-sm btn-primary shadow-sm mr-2" title="Restore All Warehouses">
                        Restore All Warehouses
                    </button>
                </form>
                <a href="#" class="btn btn-sm btn-outline-danger shadow-sm mr-2 btn-delete" data-toggle="modal" data-target="#deleteWarehouseModal" data-id="0">
                    Permanently Delete All Warehouses
                </a>
                <a href="{{ route('warehouses.index') }}" class="btn btn-sm btn-outline-primary shadow-sm">Back to Warehouses List</a>
            </div>
        </div>
        <div class="row">
            <div class="card-body">
                                    ...
                                    <td class="align-middle text-center">
                                        <div class="row justify-content-center deleted-action-section">
                                            <div class="col-auto button-action-deleted-left">
                                                <form action="{{ route('warehouses.restore', $warehouse->id) }}" method="POST">
                                                    @csrf
                                                    @method('PUT')
                                                    <button type="submit" class="btn btn-sm btn-success" title="Restore Warehouse">
...
@push('addon-script')
    ...
    <script type="text/javascript">
        ...
        $(document).on('click', '.btn-delete', function () {
            ...
            const url = `{{ route('warehouses.remove', 'warehouseId') }}`;

            $('#deleteForm').attr('action', url.replace('warehouseId', warehouseId));
        });
    </script>
@endpush
