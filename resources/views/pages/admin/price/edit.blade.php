...
@section('content')
    <div class="container-fluid">
        <div class="d-sm-flex align-items-center justify-content-between mb-2">
            <h1 class="h3 mb-0 text-gray-800 menu-title">Detail Harga - {{ $price->name }}</h1>
        </div>
        @if($errors->any())
                        ...
                        <div class="card-body">
                            <form action="{{ route('prices.update', $price->id) }}" method="POST">
                                ...
                                <div class="form-group row">
                                    ...
                                    <div class="col-6">
                                        <input type="text" class="form-control col-form-label-sm" name="name" id="name" value="{{ $price->name }}" required autofocus>
                                    </div>
                                </div>
                                <div class="form-group row">
                                    ...
                                    <span class="col-form-label text-bold">:</span>
                                    <div class="col-6">
                                        <input type="text" class="form-control col-form-label-sm" name="code" id="code" value="{{ $price->code }}" required>
                                    </div>
                                </div>
                                <div class="form-group row">
                                    ...
                                    <div class="col-3">
                                        <select class="custom-select mr-sm-2" name="type" id="type">
                                            @foreach($priceTypes as $key => $priceType)
                                                <option value="{{ $key }}" @if($price->type == $key) selected @endif>{{ $priceType }}</option>
                                            @endforeach
                                        </select>
                                        @error('type')
